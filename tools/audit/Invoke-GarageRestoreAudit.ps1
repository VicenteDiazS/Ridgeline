...
async def run_restore_audit(args):
    ...
    with tempfile.TemporaryDirectory(prefix="ridgeline-garage-restore-") as temp_dir:
        ...
        async with async_playwright() as playwright:
            ...
            restore_plan_button = page.locator("#diagnostic-activity [data-copy-garage-restore-plan]")
            preview = page.locator("#diagnostic-activity [data-garage-backup-preview]")
            assert_true(await restore_button.is_disabled(), "restore button should start disabled")
            assert_true(await restore_plan_button.is_disabled(), "restore plan button should start disabled")
            ...
            assert_true(await restore_plan_button.is_disabled(), "diagnostic activity handoff should not enable restore plan copy")
            ...
            assert_true(await restore_plan_button.is_disabled(), "invalid-only backup should not enable restore plan copy")
            ...
            assert_true(await restore_plan_button.is_enabled(), "mixed valid backup should enable restore plan copy")
            text = await preview_text(page)
            assert_true("Backup ready to restore" in text, "valid backup preview title missing")
            assert_true("Copy Restore Plan" in text, "valid backup preview should include a copy restore plan action")
            ...
            await restore_plan_button.click()
            await page.wait_for_timeout(150)
            plan_status = await status_text(page)
            assert_true("restore plan" in plan_status.lower(), "copy restore plan action should report status")
            ...
            assert_true(await restore_plan_button.is_disabled(), "restore plan button should reset disabled after restore")
