...
- Garage Recent Maintenance Notes preview completed 2026-05-17 at `garage.html#maintenance-note-preview`; it shows recent Service Prep and Maintenance Minder planner notes already saved into `general_notes` without a schema change. The panel now includes Copy Latest, per-note Copy Note, source-planner routes, Open Full Note links, live copy status, a derived Parts And Supplies Staging list, local-only Need to buy / Staged toggles, All / Need / Staged filtering, Copy Buy List, whole-list and per-note bulk staging/reset controls, and a Garage Dashboard Parts Staging count card with direct iPhone bottom-bar routing. Review this path on the real iPhone before adding larger job templates, inventory storage, restorable staging state, or remote-sync conflict behavior.
...
- Guarded Garage Notes handoffs for the Maintenance Minder Pocket Planner and Service Prep Planner completed 2026-05-16; a Garage preview for those saved notes was added 2026-05-17 and now includes copy/source/full-note handoff actions plus a no-schema Parts And Supplies Staging list derived from saved note lines with local-only Need to buy / Staged toggles, All / Need / Staged filtering, Copy Buy List for remaining purchases, and whole-list/per-note bulk staging controls. The Garage Dashboard now summarizes staging counts and routes directly to the panel from the iPhone bottom bar. Next review should use a real iPhone and the real Garage notes view before adding larger job templates, inventory fields, restorable staging data, or remote-sync conflict behavior.
