...
- Service Prep Planner completed 2026-05-16 for oil, wheel/tire, battery, and filter staging checklists, including a guarded Save action that writes checked prep items or the full card into local Garage Notes while preserving unrelated note keys.
- Maintenance Minder Pocket Planner completed 2026-05-16 near `maintenance.html#minder`; it builds copyable checklists from the existing A/B and sub-item 1-6 rows, rejects unsupported sub-code 7, routes to Quick Maintenance Update and Garage Notes, can save the generated checklist into local Garage Notes, and avoids mileage prediction or brake-fluid sub-code wording.
- Garage Recent Maintenance Notes preview completed 2026-05-17 at `garage.html#maintenance-note-preview`; it shows recent Service Prep and Maintenance Minder planner notes already saved into `general_notes` without a schema change. The panel now includes Copy Latest, per-note Copy Note, source-planner routes, Open Full Note links, live copy status, a derived Parts And Supplies Staging list, local-only Need to buy / Staged toggles, All / Need / Staged filtering, header and store-run Copy Buy List actions, Share Buy List through Web Share with clipboard fallback, whole-list and per-note bulk staging/reset controls, and a Garage Dashboard Parts Staging count card with direct iPhone bottom-bar routing. Maintenance planner Stage actions now save the current planner note and open the Garage staging panel directly with a one-visit "Just saved" receipt and latest-note highlight. The Garage staging panel also explains the local-only state boundary and why some saved planner notes appear below without producing staging items. Next high-value slice: review the Stage-to-Garage receipt and iOS share-sheet behavior on the user's iPhone before adding larger job templates, inventory storage, restorable staging state, or remote-sync conflict behavior.
...
- 2026-05-17: Maintenance-planning research for workflow ideas found recurring owner confusion about no fixed maintenance schedule, planning oil/tire/filter jobs, and checking supplies before service. Sources used only for feature inspiration, not factual vehicle data: https://www.ridgelineownersclub.com/threads/maintenance-schedule.227347/, https://www.reddit.com/r/hondaridgeline/comments/1q287ff/maintenance_schedule_am_i_missing_anything/, and https://www.reddit.com/r/hondaridgeline/comments/1sslb7j/oil_changes/. This supported the direct Maintenance planner Stage handoff to Garage staging; no forum/reddit intervals, part numbers, torque specs, fluid specs, or repair procedures were added.
...
- Guarded Garage Notes handoffs for the Maintenance Minder Pocket Planner and Service Prep Planner completed 2026-05-16; a Garage preview for those saved notes was added 2026-05-17 and now includes copy/source/full-note handoff actions plus a no-schema Parts And Supplies Staging list derived from saved note lines with local-only Need to buy / Staged toggles, All / Need / Staged filtering, Copy/Share Buy List for remaining purchases, whole-list/per-note bulk staging controls, and an iPhone-visible guide for local-only/skipped-note behavior. The Garage Dashboard now summarizes staging counts and routes directly to the panel from the iPhone bottom bar, and Maintenance planner Stage buttons now save the current planner note before opening that Garage staging panel with a one-visit receipt. Next review should use a real iPhone and the real Garage notes view before adding larger job templates, inventory fields, restorable staging data, or remote-sync conflict behavior.
