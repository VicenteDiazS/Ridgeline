...
- Parts list templates by job. First no-schema Garage staging slice completed 2026-05-17 by deriving copyable parts/supplies lists from saved maintenance planner notes, then adding local-only Need to buy / Staged toggles, All / Need / Staged filtering, Counter Mode with a next-item card, Skip This Item / Reset Skips, Mark Next Staged action, and Undo Last correction path, local-only one-off store items with quick-add supply chips, quick-kit batches, and Clear One-Offs cleanup, Copy/Share Buy List, per-saved-note Copy/Share/Save Need actions, Save Buy Note, Save Run Note, whole-list/per-note bulk staging controls, a dashboard count card, direct Maintenance planner Stage actions with a dismissible Garage handoff receipt, and a local-only/skipped-note guide for the iPhone Garage path.
