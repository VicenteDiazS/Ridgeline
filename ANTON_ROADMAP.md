...
- Main audit wrapper Garage restore coverage, completed 2026-05-16. `Invoke-SiteAudit.ps1` now runs `Invoke-GarageRestoreAudit.ps1` by default, accepts `-BrowserPath`, can skip only the legacy Edge smoke phase with `-SkipBrowserSmoke`, can skip restore with `-SkipGarageRestoreAudit`, and includes `quick-sheet.html` in its default page list.
...
- Review Garage restore with a real user backup file before adding overwrite/merge conflict choices, plus real-device review of the Diagnostics and Garage mobile density before adding more workflow surfaces. Reusable Playwright restore-validation coverage now runs through the main site-audit wrapper and checks hidden reveal ancestors plus backup/current preview labels; the next tooling slice is replacing or bypassing the flaky Edge dump-DOM smoke path with a Playwright/Chrome smoke runner.
