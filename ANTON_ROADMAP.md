...
- Parts list templates by job. First no-schema Garage staging slice completed 2026-05-17 by deriving copyable parts/supplies lists from saved maintenance planner notes, then adding local-only Need to buy / Staged toggles, All / Need / Staged filtering, Copy/Share Buy List, per-saved-note Copy/Share/Save Need actions, Save Buy Note, Save Run Note, whole-list/per-note bulk staging controls, a dashboard count card, direct Maintenance planner Stage actions with a Garage handoff receipt, and a local-only/skipped-note guide for the iPhone Garage path.
