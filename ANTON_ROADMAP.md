...
- Garage Parts And Supplies Staging, completed 2026-05-17 inside `garage.html#maintenance-note-preview`. The Garage preview now derives a copyable staging list from known saved Service Prep/Maintenance Minder planner-note lines, links to existing parts sources, keeps fitment/truck-label cautions visible, and has local-only Need to buy / Staged toggles with per-note progress counts. The Garage Dashboard now has a Parts Staging card that summarizes need/staged counts, routes directly to the preview, and updates after toggles; the iPhone contextual bottom bar, command palette, and visual site map also expose the staging route. Follow-up slices added All / Need / Staged filtering, Copy Buy List for remaining purchases, a store-run need/staged summary, whole-list Mark Need Staged / Reset Staged controls, and per-note Mark Group Staged / Reset Group controls while keeping toggle state outside the Garage backup/sync schema.
- Next safe maintenance-planning slice: review the staging checklist, bulk controls, and Copy Buy List on the real iPhone/Garage Notes path before adding larger job templates, inventory storage, restorable staging data, or remote-sync conflict behavior.
