...
- Garage Parts And Supplies Staging, completed 2026-05-17 inside `garage.html#maintenance-note-preview`. The Garage preview now derives a copyable staging list from known saved Service Prep/Maintenance Minder planner-note lines, links to existing parts sources, keeps fitment/truck-label cautions visible, and has local-only Need to buy / Staged toggles with per-note progress counts. The Garage Dashboard now has a Parts Staging card that summarizes need/staged counts, routes directly to the preview, and updates after toggles; the Garage iPhone contextual bottom bar, command palette, and visual site map also expose the staging route. Follow-up slices added All / Need / Staged filtering, Counter Mode for a one-tap need-only parts-counter view with a sticky iPhone next-item card, item-position and next-up preview, Copy Next Item, Share Next Item, Skip This Item, Reset Skips, Mark Next Staged, Undo Last correction path, and an all-staged completion panel with Save Run Note, Copy Staged List, Share Staged List, Open Final Parts, and Draft Staged Parts, Copy Buy List for remaining purchases in both the panel header and store-run summary, Share Buy List through Web Share with clipboard fallback, per-saved-note Copy Need / Share Need / Save Need controls for job-specific remaining purchases, Save Buy Note to prepend a timestamped remaining-purchases snapshot into existing Garage Notes, Save Run Note to prepend a timestamped staging-run receipt into existing Garage Notes, a store-run need/staged summary, whole-list Mark Need Staged / Reset Staged controls, per-note Mark Group Staged / Reset Group controls, local-only one-off store item entry/removal for extras not detected from saved notes, one-tap quick-add chips for common one-off supplies, one-tap quick kits for Oil run / Filter run / Electrical check / Cleanup helper batches, Clear One-Offs cleanup for local helper items, a Final Part Numbers panel that can draft from need-to-buy or staged lines before saving user-confirmed entries into the existing syncable Truck Profile `parts_notes`, and a compact staging guide plus per-note staging-state chips that explain derived line counts, local-only Need/Staged state, one-off helper state, and saved notes that do not produce staging lines while keeping helper state outside the Garage backup/sync schema. Maintenance planner Stage in Garage actions now save the current Service Prep or Maintenance Minder planner note, open Garage staging directly, show a one-visit "Just saved" receipt that can be dismissed, highlight the newest saved note, and Maintenance's iPhone contextual bottom bar now routes directly to Garage staging. The Garage receipt previews the just-saved note's current need-to-buy lines and excludes Maintenance Minder brake-fluid calendar cautions from staging/need-to-buy parsing.
- Next safe maintenance-planning slice: review the Stage-to-Garage bottom-bar route, receipt preview/dismiss behavior, sticky Counter Mode completion / Copy Staged List / Share Staged List / Draft Staged Parts / Save Run Note handoff, item-position / next-up / Skip This Item / Reset Skips / Copy Next Item / Share Next Item / Undo Last / Mark Next Staged flow, one-off quick kits/clear-one-offs/removal, per-note Need actions, Save Buy Note wording, staging checklist, Share Buy List / Share Staged List / Share Next Item iOS share-sheet behavior, and bulk controls on the user's real iPhone before adding larger job templates, inventory storage, restorable staging data, or remote-sync conflict behavior.
...
- Parts list templates by job. First no-schema Garage staging slice completed 2026-05-17 by deriving copyable parts/supplies lists from saved maintenance planner notes, then adding local-only Need to buy / Staged toggles, All / Need / Staged filtering, Counter Mode with a next-item card, Skip This Item / Reset Skips, Mark Next Staged action, Undo Last correction path, local-only one-off store items with quick-add supply chips, quick-kit batches, Clear One-Offs cleanup, Copy/Share Buy List, per-saved-note Copy/Share/Save Need actions, Save Buy Note, Save Run Note, whole-list/per-note bulk staging controls, a dashboard count card, direct Maintenance planner Stage actions with a dismissible Garage handoff receipt, a Final Part Numbers staged-list draft path, and a local-only/skipped-note guide for the iPhone Garage path.
- Service Prep Planner checklist cards completed 2026-05-16 on `maintenance.html#service-prep`, including guarded local Garage Notes save for checked prep items; Maintenance Minder Pocket Planner completed 2026-05-16 on `maintenance.html#minder-pocket-planner`, including a guarded local Garage Notes save for built checklist text; Garage Recent Maintenance Notes preview, copy/source actions, derived parts/supplies staging, local-only staging status toggles, filter/copy/share-buy-list actions, per-saved-note Copy/Share/Save Need actions, Save Buy Note, Save Run Note, store-run Copy/Share Buy List access, direct Garage dashboard/bottom-bar staging routes, and Maintenance save-and-open Stage actions with a Garage receipt completed 2026-05-17. Future work should review the saved-note confirmation path and iOS share-sheet behavior on the user's iPhone before adding larger parts or inventory flows.
