...
- Garage Parts And Supplies Staging, completed 2026-05-17 inside `garage.html#maintenance-note-preview`. The Garage preview now derives a copyable staging list from known saved Service Prep/Maintenance Minder planner-note lines, links to existing parts sources, keeps fitment/truck-label cautions visible, and has local-only Need to buy / Staged toggles with per-note progress counts. The Garage Dashboard now has a Parts Staging card that summarizes need/staged counts, routes directly to the preview, and updates after toggles; the iPhone contextual bottom bar, command palette, and visual site map also expose the staging route. Follow-up slices added All / Need / Staged filtering, Copy Buy List for remaining purchases in both the panel header and store-run summary, a store-run need/staged summary, whole-list Mark Need Staged / Reset Staged controls, and per-note Mark Group Staged / Reset Group controls while keeping toggle state outside the Garage backup/sync schema. Maintenance planner Stage actions now save the current Service Prep or Maintenance Minder planner note and open Garage staging directly.
- Next safe maintenance-planning slice: clarify local-only staging state and empty/partial derived-staging explanations in Garage before adding larger job templates, inventory storage, restorable staging data, or remote-sync conflict behavior. Real iPhone review should still check the staging checklist, store-run Copy Buy List, bulk controls, Stage route, and bottom-bar route.
...
- Parts list templates by job. First no-schema Garage staging slice completed 2026-05-17 by deriving copyable parts/supplies lists from saved maintenance planner notes, then adding local-only Need to buy / Staged toggles, All / Need / Staged filtering, Copy Buy List, whole-list/per-note bulk staging controls, a dashboard count card, and direct Maintenance planner Stage actions for the iPhone Garage path.
