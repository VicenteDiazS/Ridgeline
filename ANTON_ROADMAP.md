...
- Garage Parts And Supplies Staging, completed 2026-05-17 inside `garage.html#maintenance-note-preview`. The Garage preview now derives a copyable staging list from known saved Service Prep/Maintenance Minder planner-note lines, links to existing parts sources, and keeps fitment/truck-label cautions visible without changing Garage storage or adding vehicle facts.
- Next safe maintenance-planning slice: review the derived staging panel on the real iPhone/Garage Notes path before adding explicit "need to buy / already staged" state, larger job templates, inventory storage, or remote-sync conflict behavior.
...
- Parts list templates by job. First no-schema Garage staging slice completed 2026-05-17 by deriving copyable parts/supplies lists from saved maintenance planner notes.
- Service Prep Planner checklist cards completed 2026-05-16 on `maintenance.html#service-prep`, including guarded local Garage Notes save for checked prep items; Maintenance Minder Pocket Planner completed 2026-05-16 on `maintenance.html#minder-pocket-planner`, including a guarded local Garage Notes save for built checklist text; Garage Recent Maintenance Notes preview, copy/source actions, and derived parts/supplies staging completed 2026-05-17. Future work should review the saved-note confirmation path on the user's iPhone before adding larger parts or inventory flows.
