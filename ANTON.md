...
- When Counter Mode reaches an all-staged state, keep end-of-run actions in the sticky panel: Undo Last for correction, Save Run Note for a local Garage Notes receipt, and a direct final-part-number handoff into the existing Truck Profile path before considering any inventory schema.
- Final-part-number handoffs should work after the parts-counter run is complete: keep a staged-list draft path available in the completed sticky panel when all need-to-buy lines have been marked staged, and still require user-entered confirmation before saving to Truck Profile.
- Completed Counter Mode lists should also stay copyable/shareable from the sticky all-staged panel, using derived staged-list exports only; do not add syncable inventory or job schema just to hand off the completed run.
