...
- `Invoke-SiteAudit.ps1` now runs the Garage restore Playwright audit by default before the legacy Edge dump-DOM smoke phase; use `-SkipBrowserSmoke` for targeted wrapper verification when dump-DOM is blocked, and use `-SkipGarageRestoreAudit` only for quick non-Garage checks.
